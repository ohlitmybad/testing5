...
function getCurrentMetricValue(parsedData, selectedPlayer, metric) {
    ...
    const playerData = parsedData.find(p => 
        ...
        p.position === displayLeague(selectedPlayer.position) && 
        p.league === displayLeague(selectedPlayer.league)
    );
...
function updateCurrentMetricValue(parsedData, selectedPlayer, metric) {
    ...
    const playerData = parsedData.find(p => 
        p.player === selectedPlayer.player && 
        p.position === positionToNumber[displayLeague(selectedPlayer.position)] && 
        p.league === leagueToNumber[displayLeague(selectedPlayer.league)]
    );
...
// Metric: defActions
const samePositionAndLeagueActions = calculateRankForMetric(filteredData, 'defActions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueActionsWithMinutes = calculateRankForMetric(filteredData, 'defActions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, defActions: Math.round(p.defActions * p.minutes / 90)}));
const positionRankActions = calculateRankForMetric(filteredData, 'defActions', p => p.position === displayLeague(selectedPlayer.position));
const positionRankActionsWithMinutes = calculateRankForMetric(filteredData, 'defActions', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, defActions: Math.round(p.defActions * p.minutes / 90)}));
const leagueRankActions = calculateRankForMetric(filteredData, 'defActions', p => p.league === displayLeague(selectedPlayer.league));
const leagueRankActionsWithMinutes = calculateRankForMetric(filteredData, 'defActions', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, defActions: Math.round(p.defActions * p.minutes / 90)}));
...
// Metric: defDuels
const positionRankDuelsWithMinutes = calculateRankForMetric(filteredData, 'defDuels', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, defDuels: Math.round(p.defDuels * p.minutes / 90)}));
const positionRankDuels = calculateRankForMetric(filteredData, 'defDuels', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueDuels = calculateRankForMetric(filteredData, 'defDuels', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDuelsWithMinutes = calculateRankForMetric(filteredData, 'defDuels', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, defDuels: Math.round(p.defDuels * p.minutes / 90)}));
const leagueRankDuels = calculateRankForMetric(filteredData, 'defDuels', p => p.league === displayLeague(selectedPlayer.league));
const leagueRankDuelsWithMinutes = calculateRankForMetric(filteredData, 'defDuels', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, defDuels: Math.round(p.defDuels * p.minutes / 90)}));
...
const leagueRankAerialDuels = calculateRankForMetric(filteredData, 'aerialDuels', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAerialDuelsWithMinutes = calculateRankForMetric(filteredData, 'aerialDuels', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, aerialDuels: Math.round(p.aerialDuels * p.minutes / 90)}));
const positionRankAerialDuels = calculateRankForMetric(filteredData, 'aerialDuels', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAerialDuelsWithMinutes = calculateRankForMetric(filteredData, 'aerialDuels', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, aerialDuels: Math.round(p.aerialDuels * p.minutes / 90)}));
const samePositionAndLeagueAerialDuels = calculateRankForMetric(filteredData, 'aerialDuels', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAerialDuelsWithMinutes = calculateRankForMetric(filteredData, 'aerialDuels', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, aerialDuels: Math.round(p.aerialDuels * p.minutes / 90)}));


// Metric: slidingTackles
const allCsvRankSlidingTackles = calculateRankForMetric(filteredData, 'slidingTackles');
const leagueRankSlidingTackles = calculateRankForMetric(filteredData, 'slidingTackles', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankSlidingTacklesWithMinutes = calculateRankForMetric(filteredData, 'slidingTackles', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, slidingTackles: Math.round(p.slidingTackles * p.minutes / 90)}));
const positionRankSlidingTackles = calculateRankForMetric(filteredData, 'slidingTackles', p => p.position === displayLeague(selectedPlayer.position));
const positionRankSlidingTacklesWithMinutes = calculateRankForMetric(filteredData, 'slidingTackles', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, slidingTackles: Math.round(p.slidingTackles * p.minutes / 90)}));
const samePositionAndLeagueSlidingTackles = calculateRankForMetric(filteredData, 'slidingTackles', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueSlidingTacklesWithMinutes = calculateRankForMetric(filteredData, 'slidingTackles', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, slidingTackles: Math.round(p.slidingTackles * p.minutes / 90)}));

// Metric: pAdjSlidingTackles
const allCsvRankPAdjSlidingTackles = calculateRankForMetric(filteredData, 'pAdjSlidingTackles');
const leagueRankPAdjSlidingTackles = calculateRankForMetric(filteredData, 'pAdjSlidingTackles', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankPAdjSlidingTacklesWithMinutes = calculateRankForMetric(filteredData, 'pAdjSlidingTackles');
const leagueRankPAdjSlidingTacklesWithMinutes = calculateRankForMetric(filteredData, 'pAdjSlidingTackles', p => p.league === displayLeague(selectedPlayer.league));
const positionRankPAdjSlidingTackles = calculateRankForMetric(filteredData, 'pAdjSlidingTackles', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPAdjSlidingTacklesWithMinutes = calculateRankForMetric(filteredData, 'pAdjSlidingTackles', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeaguePAdjSlidingTackles = calculateRankForMetric(filteredData, 'pAdjSlidingTackles', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePAdjSlidingTacklesWithMinutes = calculateRankForMetric(filteredData, 'pAdjSlidingTackles', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: shotsBlocked
const allCsvRankShotsBlocked = calculateRankForMetric(filteredData, 'shotsBlocked');
const leagueRankShotsBlocked = calculateRankForMetric(filteredData, 'shotsBlocked', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankShotsBlockedWithMinutes = calculateRankForMetric(filteredData, 'shotsBlocked', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, shotsBlocked: Math.round(p.shotsBlocked * p.minutes / 90)}));
const positionRankShotsBlocked = calculateRankForMetric(filteredData, 'shotsBlocked', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShotsBlockedWithMinutes = calculateRankForMetric(filteredData, 'shotsBlocked', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, shotsBlocked: Math.round(p.shotsBlocked * p.minutes / 90)}));
const samePositionAndLeagueShotsBlocked = calculateRankForMetric(filteredData, 'shotsBlocked', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShotsBlockedWithMinutes = calculateRankForMetric(filteredData, 'shotsBlocked', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, shotsBlocked: Math.round(p.shotsBlocked * p.minutes / 90)}));

// Metric: interceptions
const allCsvRankInterceptions = calculateRankForMetric(filteredData, 'interceptions');
const leagueRankInterceptions = calculateRankForMetric(filteredData, 'interceptions', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankInterceptionsWithMinutes = calculateRankForMetric(filteredData, 'interceptions', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, interceptions: Math.round(p.interceptions * p.minutes / 90)}));
const positionRankInterceptions = calculateRankForMetric(filteredData, 'interceptions', p => p.position === displayLeague(selectedPlayer.position));
const positionRankInterceptionsWithMinutes = calculateRankForMetric(filteredData, 'interceptions', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, interceptions: Math.round(p.interceptions * p.minutes / 90)}));
const samePositionAndLeagueInterceptions = calculateRankForMetric(filteredData, 'interceptions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueInterceptionsWithMinutes = calculateRankForMetric(filteredData, 'interceptions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, interceptions: Math.round(p.interceptions * p.minutes / 90)}));


// Metric: pAdjInterceptions
const allCsvRankPAdjInterceptions = calculateRankForMetric(filteredData, 'pAdjInterceptions');
const leagueRankPAdjInterceptions = calculateRankForMetric(filteredData, 'pAdjInterceptions', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankPAdjInterceptionsWithMinutes = calculateRankForMetric(filteredData, 'pAdjInterceptions');
const leagueRankPAdjInterceptionsWithMinutes = calculateRankForMetric(filteredData, 'pAdjInterceptions', p => p.league === displayLeague(selectedPlayer.league));
const positionRankPAdjInterceptions = calculateRankForMetric(filteredData, 'pAdjInterceptions', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPAdjInterceptionsWithMinutes = calculateRankForMetric(filteredData, 'pAdjInterceptions', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeaguePAdjInterceptions = calculateRankForMetric(filteredData, 'pAdjInterceptions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePAdjInterceptionsWithMinutes = calculateRankForMetric(filteredData, 'pAdjInterceptions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
...
const leagueRankSuccessfulAttackingActions = calculateRankForMetric(filteredData, 'successfulAttackingActions', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankSuccessfulAttackingActionsWithMinutes = calculateRankForMetric(filteredData, 'successfulAttackingActions', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, successfulAttackingActions: Math.round(p.successfulAttackingActions * p.minutes / 90)}));
const positionRankSuccessfulAttackingActions = calculateRankForMetric(filteredData, 'successfulAttackingActions', p => p.position === displayLeague(selectedPlayer.position));
const positionRankSuccessfulAttackingActionsWithMinutes = calculateRankForMetric(filteredData, 'successfulAttackingActions', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, successfulAttackingActions: Math.round(p.successfulAttackingActions * p.minutes / 90)}));
const samePositionAndLeagueSuccessfulAttackingActions = calculateRankForMetric(filteredData, 'successfulAttackingActions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueSuccessfulAttackingActionsWithMinutes = calculateRankForMetric(filteredData, 'successfulAttackingActions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, successfulAttackingActions: Math.round(p.successfulAttackingActions * p.minutes / 90)}));

// Metric: goals
const allCsvRankGoals = calculateRankForMetric(filteredData, 'goals');
const leagueRankGoals = calculateRankForMetric(filteredData, 'goals', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankGoalsWithMinutes = calculateRankForMetric(filteredData, 'goals', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, goals: Math.round(p.goals * p.minutes / 90)}));
const positionRankGoals = calculateRankForMetric(filteredData, 'goals', p => p.position === displayLeague(selectedPlayer.position));
const positionRankGoalsWithMinutes = calculateRankForMetric(filteredData, 'goals', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, goals: Math.round(p.goals * p.minutes / 90)}));
const samePositionAndLeagueGoals = calculateRankForMetric(filteredData, 'goals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueGoalsWithMinutes = calculateRankForMetric(filteredData, 'goals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, goals: Math.round(p.goals * p.minutes / 90)}));

// Metric: nonPenaltyGoals
const allCsvRankNonPenaltyGoals = calculateRankForMetric(filteredData, 'nonPenaltyGoals');
const leagueRankNonPenaltyGoals = calculateRankForMetric(filteredData, 'nonPenaltyGoals', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankNonPenaltyGoalsWithMinutes = calculateRankForMetric(filteredData, 'nonPenaltyGoals', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, nonPenaltyGoals: Math.round(p.nonPenaltyGoals * p.minutes / 90)}));
const positionRankNonPenaltyGoals = calculateRankForMetric(filteredData, 'nonPenaltyGoals', p => p.position === displayLeague(selectedPlayer.position));
const positionRankNonPenaltyGoalsWithMinutes = calculateRankForMetric(filteredData, 'nonPenaltyGoals', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, nonPenaltyGoals: Math.round(p.nonPenaltyGoals * p.minutes / 90)}));
const samePositionAndLeagueNonPenaltyGoals = calculateRankForMetric(filteredData, 'nonPenaltyGoals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueNonPenaltyGoalsWithMinutes = calculateRankForMetric(filteredData, 'nonPenaltyGoals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, nonPenaltyGoals: Math.round(p.nonPenaltyGoals * p.minutes / 90)}));

// Metric: xG
const allCsvRankXG = calculateRankForMetric(filteredData, 'xG');
const leagueRankXG = calculateRankForMetric(filteredData, 'xG', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankXGWithMinutes = calculateRankForMetric(filteredData, 'xG', p => true, p => ({...p, xG: p.xG * p.minutes}));
const leagueRankXGWithMinutes = calculateRankForMetric(filteredData, 'xG', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, xG: p.xG * p.minutes}));
const positionRankXG = calculateRankForMetric(filteredData, 'xG', p => p.position === displayLeague(selectedPlayer.position));
const positionRankXGWithMinutes = calculateRankForMetric(filteredData, 'xG', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, xG: p.xG * p.minutes}));
const samePositionAndLeagueXG = calculateRankForMetric(filteredData, 'xG', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueXGWithMinutes = calculateRankForMetric(filteredData, 'xG', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, xG: p.xG * p.minutes}));

// Metric: headGoals
const allCsvRankHeadGoals = calculateRankForMetric(filteredData, 'headGoals');
const leagueRankHeadGoals = calculateRankForMetric(filteredData, 'headGoals', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankHeadGoalsWithMinutes = calculateRankForMetric(filteredData, 'headGoals', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, headGoals: Math.round(p.headGoals * p.minutes / 90)}));
const positionRankHeadGoals = calculateRankForMetric(filteredData, 'headGoals', p => p.position === displayLeague(selectedPlayer.position));
const positionRankHeadGoalsWithMinutes = calculateRankForMetric(filteredData, 'headGoals', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, headGoals: Math.round(p.headGoals * p.minutes / 90)}));
const samePositionAndLeagueHeadGoals = calculateRankForMetric(filteredData, 'headGoals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueHeadGoalsWithMinutes = calculateRankForMetric(filteredData, 'headGoals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, headGoals: Math.round(p.headGoals * p.minutes / 90)}));

// Metric: shots
const allCsvRankShots = calculateRankForMetric(filteredData, 'shots');
const leagueRankShots = calculateRankForMetric(filteredData, 'shots', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankShotsWithMinutes = calculateRankForMetric(filteredData, 'shots', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, shots: Math.round(p.shots * p.minutes / 90)}));
const positionRankShots = calculateRankForMetric(filteredData, 'shots', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShotsWithMinutes = calculateRankForMetric(filteredData, 'shots', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, shots: Math.round(p.shots * p.minutes / 90)}));
const samePositionAndLeagueShots = calculateRankForMetric(filteredData, 'shots', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShotsWithMinutes = calculateRankForMetric(filteredData, 'shots', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, shots: Math.round(p.shots * p.minutes / 90)}));

// Metric: assists
const allCsvRankAssists = calculateRankForMetric(filteredData, 'assists');
const leagueRankAssists = calculateRankForMetric(filteredData, 'assists', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAssistsWithMinutes = calculateRankForMetric(filteredData, 'assists', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, assists: Math.round(p.assists * p.minutes / 90)}));
const positionRankAssists = calculateRankForMetric(filteredData, 'assists', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAssistsWithMinutes = calculateRankForMetric(filteredData, 'assists', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, assists: Math.round(p.assists * p.minutes / 90)}));
const samePositionAndLeagueAssists = calculateRankForMetric(filteredData, 'assists', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAssistsWithMinutes = calculateRankForMetric(filteredData, 'assists', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, assists: Math.round(p.assists * p.minutes / 90)}));

// Metric: crosses
const allCsvRankCrosses = calculateRankForMetric(filteredData, 'crosses');
const leagueRankCrosses = calculateRankForMetric(filteredData, 'crosses', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankCrossesWithMinutes = calculateRankForMetric(filteredData, 'crosses', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, crosses: Math.round(p.crosses * p.minutes / 90)}));
const positionRankCrosses = calculateRankForMetric(filteredData, 'crosses', p => p.position === displayLeague(selectedPlayer.position));
const positionRankCrossesWithMinutes = calculateRankForMetric(filteredData, 'crosses', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, crosses: Math.round(p.crosses * p.minutes / 90)}));
const samePositionAndLeagueCrosses = calculateRankForMetric(filteredData, 'crosses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueCrossesWithMinutes = calculateRankForMetric(filteredData, 'crosses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, crosses: Math.round(p.crosses * p.minutes / 90)}));

// Metric: crossesToGoalieBox
const allCsvRankCrossesToGoalieBox = calculateRankForMetric(filteredData, 'crossesToGoalieBox');
const leagueRankCrossesToGoalieBox = calculateRankForMetric(filteredData, 'crossesToGoalieBox', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankCrossesToGoalieBoxWithMinutes = calculateRankForMetric(filteredData, 'crossesToGoalieBox', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, crossesToGoalieBox: Math.round(p.crossesToGoalieBox * p.minutes / 90)}));
const positionRankCrossesToGoalieBox = calculateRankForMetric(filteredData, 'crossesToGoalieBox', p => p.position === displayLeague(selectedPlayer.position));
const positionRankCrossesToGoalieBoxWithMinutes = calculateRankForMetric(filteredData, 'crossesToGoalieBox', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, crossesToGoalieBox: Math.round(p.crossesToGoalieBox * p.minutes / 90)}));
const samePositionAndLeagueCrossesToGoalieBox = calculateRankForMetric(filteredData, 'crossesToGoalieBox', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueCrossesToGoalieBoxWithMinutes = calculateRankForMetric(filteredData, 'crossesToGoalieBox', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, crossesToGoalieBox: Math.round(p.crossesToGoalieBox * p.minutes / 90)}));

// Metric: dribbles
const allCsvRankDribbles = calculateRankForMetric(filteredData, 'dribbles');
const leagueRankDribbles = calculateRankForMetric(filteredData, 'dribbles', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankDribblesWithMinutes = calculateRankForMetric(filteredData, 'dribbles', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, dribbles: Math.round(p.dribbles * p.minutes / 90)}));
const positionRankDribbles = calculateRankForMetric(filteredData, 'dribbles', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDribblesWithMinutes = calculateRankForMetric(filteredData, 'dribbles', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, dribbles: Math.round(p.dribbles * p.minutes / 90)}));
const samePositionAndLeagueDribbles = calculateRankForMetric(filteredData, 'dribbles', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDribblesWithMinutes = calculateRankForMetric(filteredData, 'dribbles', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, dribbles: Math.round(p.dribbles * p.minutes / 90)}));

// Metric: offensiveDuels
const allCsvRankOffensiveDuels = calculateRankForMetric(filteredData, 'offensiveDuels');
const leagueRankOffensiveDuels = calculateRankForMetric(filteredData, 'offensiveDuels', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankOffensiveDuelsWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuels', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, offensiveDuels: Math.round(p.offensiveDuels * p.minutes / 90)}));
const positionRankOffensiveDuels = calculateRankForMetric(filteredData, 'offensiveDuels', p => p.position === displayLeague(selectedPlayer.position));
const positionRankOffensiveDuelsWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuels', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, offensiveDuels: Math.round(p.offensiveDuels * p.minutes / 90)}));
const samePositionAndLeagueOffensiveDuels = calculateRankForMetric(filteredData, 'offensiveDuels', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueOffensiveDuelsWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuels', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, offensiveDuels: Math.round(p.offensiveDuels * p.minutes / 90)}));

// Metric: touchesInBox
const allCsvRankTouchesInBox = calculateRankForMetric(filteredData, 'touchesInBox');
const leagueRankTouchesInBox = calculateRankForMetric(filteredData, 'touchesInBox', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankTouchesInBoxWithMinutes = calculateRankForMetric(filteredData, 'touchesInBox', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, touchesInBox: Math.round(p.touchesInBox * p.minutes / 90)}));
const positionRankTouchesInBox = calculateRankForMetric(filteredData, 'touchesInBox', p => p.position === displayLeague(selectedPlayer.position));
const positionRankTouchesInBoxWithMinutes = calculateRankForMetric(filteredData, 'touchesInBox', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, touchesInBox: Math.round(p.touchesInBox * p.minutes / 90)}));
const samePositionAndLeagueTouchesInBox = calculateRankForMetric(filteredData, 'touchesInBox', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueTouchesInBoxWithMinutes = calculateRankForMetric(filteredData, 'touchesInBox', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, touchesInBox: Math.round(p.touchesInBox * p.minutes / 90)}));

// Metric: progressiveRuns
const allCsvRankProgressiveRuns = calculateRankForMetric(filteredData, 'progressiveRuns');
const leagueRankProgressiveRuns = calculateRankForMetric(filteredData, 'progressiveRuns', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankProgressiveRunsWithMinutes = calculateRankForMetric(filteredData, 'progressiveRuns', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, progressiveRuns: Math.round(p.progressiveRuns * p.minutes / 90)}));
const positionRankProgressiveRuns = calculateRankForMetric(filteredData, 'progressiveRuns', p => p.position === displayLeague(selectedPlayer.position));
const positionRankProgressiveRunsWithMinutes = calculateRankForMetric(filteredData, 'progressiveRuns', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, progressiveRuns: Math.round(p.progressiveRuns * p.minutes / 90)}));
const samePositionAndLeagueProgressiveRuns = calculateRankForMetric(filteredData, 'progressiveRuns', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueProgressiveRunsWithMinutes = calculateRankForMetric(filteredData, 'progressiveRuns', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, progressiveRuns: Math.round(p.progressiveRuns * p.minutes / 90)}));

// Metric: accelerations
const allCsvRankAccelerations = calculateRankForMetric(filteredData, 'accelerations');
const leagueRankAccelerations = calculateRankForMetric(filteredData, 'accelerations', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccelerationsWithMinutes = calculateRankForMetric(filteredData, 'accelerations', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, accelerations: Math.round(p.accelerations * p.minutes / 90)}));
const positionRankAccelerations = calculateRankForMetric(filteredData, 'accelerations', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccelerationsWithMinutes = calculateRankForMetric(filteredData, 'accelerations', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, accelerations: Math.round(p.accelerations * p.minutes / 90)}));
const samePositionAndLeagueAccelerations = calculateRankForMetric(filteredData, 'accelerations', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccelerationsWithMinutes = calculateRankForMetric(filteredData, 'accelerations', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, accelerations: Math.round(p.accelerations * p.minutes / 90)}));

// Metric: foulsSuffered
const allCsvRankFoulsSuffered = calculateRankForMetric(filteredData, 'foulsSuffered');
const leagueRankFoulsSuffered = calculateRankForMetric(filteredData, 'foulsSuffered', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankFoulsSufferedWithMinutes = calculateRankForMetric(filteredData, 'foulsSuffered', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, foulsSuffered: Math.round(p.foulsSuffered * p.minutes / 90)}));
const positionRankFoulsSuffered = calculateRankForMetric(filteredData, 'foulsSuffered', p => p.position === displayLeague(selectedPlayer.position));
const positionRankFoulsSufferedWithMinutes = calculateRankForMetric(filteredData, 'foulsSuffered', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, foulsSuffered: Math.round(p.foulsSuffered * p.minutes / 90)}));
const samePositionAndLeagueFoulsSuffered = calculateRankForMetric(filteredData, 'foulsSuffered', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueFoulsSufferedWithMinutes = calculateRankForMetric(filteredData, 'foulsSuffered', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, foulsSuffered: Math.round(p.foulsSuffered * p.minutes / 90)}));

// Metric: passes
const allCsvRankPasses = calculateRankForMetric(filteredData, 'passes');
const leagueRankPasses = calculateRankForMetric(filteredData, 'passes', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankPassesWithMinutes = calculateRankForMetric(filteredData, 'passes', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, passes: Math.round(p.passes * p.minutes / 90)}));
const positionRankPasses = calculateRankForMetric(filteredData, 'passes', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPassesWithMinutes = calculateRankForMetric(filteredData, 'passes', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, passes: Math.round(p.passes * p.minutes / 90)}));
const samePositionAndLeaguePasses = calculateRankForMetric(filteredData, 'passes', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePassesWithMinutes = calculateRankForMetric(filteredData, 'passes', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, passes: Math.round(p.passes * p.minutes / 90)}));

// Metric: forwardPasses
const allCsvRankForwardPasses = calculateRankForMetric(filteredData, 'forwardPasses');
const leagueRankForwardPasses = calculateRankForMetric(filteredData, 'forwardPasses', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankForwardPassesWithMinutes = calculateRankForMetric(filteredData, 'forwardPasses', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, forwardPasses: Math.round(p.forwardPasses * p.minutes / 90)}));
const positionRankForwardPasses = calculateRankForMetric(filteredData, 'forwardPasses', p => p.position === displayLeague(selectedPlayer.position));
const positionRankForwardPassesWithMinutes = calculateRankForMetric(filteredData, 'forwardPasses', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, forwardPasses: Math.round(p.forwardPasses * p.minutes / 90)}));
const samePositionAndLeagueForwardPasses = calculateRankForMetric(filteredData, 'forwardPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueForwardPassesWithMinutes = calculateRankForMetric(filteredData, 'forwardPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, forwardPasses: Math.round(p.forwardPasses * p.minutes / 90)}));

// Metric: shortMediumPasses
const allCsvRankShortMediumPasses = calculateRankForMetric(filteredData, 'shortMediumPasses');
const leagueRankShortMediumPasses = calculateRankForMetric(filteredData, 'shortMediumPasses', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankShortMediumPassesWithMinutes = calculateRankForMetric(filteredData, 'shortMediumPasses', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, shortMediumPasses: Math.round(p.shortMediumPasses * p.minutes / 90)}));
const positionRankShortMediumPasses = calculateRankForMetric(filteredData, 'shortMediumPasses', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShortMediumPassesWithMinutes = calculateRankForMetric(filteredData, 'shortMediumPasses', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, shortMediumPasses: Math.round(p.shortMediumPasses * p.minutes / 90)}));
const samePositionAndLeagueShortMediumPasses = calculateRankForMetric(filteredData, 'shortMediumPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShortMediumPassesWithMinutes = calculateRankForMetric(filteredData, 'shortMediumPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, shortMediumPasses: Math.round(p.shortMediumPasses * p.minutes / 90)}));

// Metric: longPasses
const allCsvRankLongPasses = calculateRankForMetric(filteredData, 'longPasses');
const leagueRankLongPasses = calculateRankForMetric(filteredData, 'longPasses', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankLongPassesWithMinutes = calculateRankForMetric(filteredData, 'longPasses', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, longPasses: Math.round(p.longPasses * p.minutes / 90)}));
const positionRankLongPasses = calculateRankForMetric(filteredData, 'longPasses', p => p.position === displayLeague(selectedPlayer.position));
const positionRankLongPassesWithMinutes = calculateRankForMetric(filteredData, 'longPasses', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, longPasses: Math.round(p.longPasses * p.minutes / 90)}));
const samePositionAndLeagueLongPasses = calculateRankForMetric(filteredData, 'longPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueLongPassesWithMinutes = calculateRankForMetric(filteredData, 'longPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, longPasses: Math.round(p.longPasses * p.minutes / 90)}));


// Metric: xA (Expected Assists)
const allCsvRankXA = calculateRankForMetric(filteredData, 'xA');
const leagueRankXA = calculateRankForMetric(filteredData, 'xA', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankXAWithMinutes = calculateRankForMetric(filteredData, 'xA', p => true, p => ({...p, xA: p.xA * p.minutes}));
const leagueRankXAWithMinutes = calculateRankForMetric(filteredData, 'xA', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, xA: p.xA * p.minutes}));
const positionRankXA = calculateRankForMetric(filteredData, 'xA', p => p.position === displayLeague(selectedPlayer.position));
const positionRankXAWithMinutes = calculateRankForMetric(filteredData, 'xA', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, xA: p.xA * p.minutes}));
const samePositionAndLeagueXA = calculateRankForMetric(filteredData, 'xA', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueXAWithMinutes = calculateRankForMetric(filteredData, 'xA', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, xA: p.xA * p.minutes}));

// Metric: shotAssists
const allCsvRankShotAssists = calculateRankForMetric(filteredData, 'shotAssists');
const leagueRankShotAssists = calculateRankForMetric(filteredData, 'shotAssists', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankShotAssistsWithMinutes = calculateRankForMetric(filteredData, 'shotAssists', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, shotAssists: Math.round(p.shotAssists * p.minutes / 90)}));
const positionRankShotAssists = calculateRankForMetric(filteredData, 'shotAssists', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShotAssistsWithMinutes = calculateRankForMetric(filteredData, 'shotAssists', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, shotAssists: Math.round(p.shotAssists * p.minutes / 90)}));
const samePositionAndLeagueShotAssists = calculateRankForMetric(filteredData, 'shotAssists', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShotAssistsWithMinutes = calculateRankForMetric(filteredData, 'shotAssists', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, shotAssists: Math.round(p.shotAssists * p.minutes / 90)}));

// Metric: keyPasses
const allCsvRankKeyPasses = calculateRankForMetric(filteredData, 'keyPasses');
const leagueRankKeyPasses = calculateRankForMetric(filteredData, 'keyPasses', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankKeyPassesWithMinutes = calculateRankForMetric(filteredData, 'keyPasses', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, keyPasses: Math.round(p.keyPasses * p.minutes / 90)}));
const positionRankKeyPasses = calculateRankForMetric(filteredData, 'keyPasses', p => p.position === displayLeague(selectedPlayer.position));
const positionRankKeyPassesWithMinutes = calculateRankForMetric(filteredData, 'keyPasses', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, keyPasses: Math.round(p.keyPasses * p.minutes / 90)}));
const samePositionAndLeagueKeyPasses = calculateRankForMetric(filteredData, 'keyPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueKeyPassesWithMinutes = calculateRankForMetric(filteredData, 'keyPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, keyPasses: Math.round(p.keyPasses * p.minutes / 90)}));

// Metric: passesToFinalThird
const allCsvRankPassesToFinalThird = calculateRankForMetric(filteredData, 'passesToFinalThird');
const leagueRankPassesToFinalThird = calculateRankForMetric(filteredData, 'passesToFinalThird', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankPassesToFinalThirdWithMinutes = calculateRankForMetric(filteredData, 'passesToFinalThird', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, passesToFinalThird: Math.round(p.passesToFinalThird * p.minutes / 90)}));
const positionRankPassesToFinalThird = calculateRankForMetric(filteredData, 'passesToFinalThird', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPassesToFinalThirdWithMinutes = calculateRankForMetric(filteredData, 'passesToFinalThird', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, passesToFinalThird: Math.round(p.passesToFinalThird * p.minutes / 90)}));
const samePositionAndLeaguePassesToFinalThird = calculateRankForMetric(filteredData, 'passesToFinalThird', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePassesToFinalThirdWithMinutes = calculateRankForMetric(filteredData, 'passesToFinalThird', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, passesToFinalThird: Math.round(p.passesToFinalThird * p.minutes / 90)}));

// Metric: passesToPenaltyArea
const allCsvRankPassesToPenaltyArea = calculateRankForMetric(filteredData, 'passesToPenaltyArea');
const leagueRankPassesToPenaltyArea = calculateRankForMetric(filteredData, 'passesToPenaltyArea', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankPassesToPenaltyAreaWithMinutes = calculateRankForMetric(filteredData, 'passesToPenaltyArea', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, passesToPenaltyArea: Math.round(p.passesToPenaltyArea * p.minutes / 90)}));
const positionRankPassesToPenaltyArea = calculateRankForMetric(filteredData, 'passesToPenaltyArea', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPassesToPenaltyAreaWithMinutes = calculateRankForMetric(filteredData, 'passesToPenaltyArea', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, passesToPenaltyArea: Math.round(p.passesToPenaltyArea * p.minutes / 90)}));
const samePositionAndLeaguePassesToPenaltyArea = calculateRankForMetric(filteredData, 'passesToPenaltyArea', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePassesToPenaltyAreaWithMinutes = calculateRankForMetric(filteredData, 'passesToPenaltyArea', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, passesToPenaltyArea: Math.round(p.passesToPenaltyArea * p.minutes / 90)}));

// Metric: throughPasses
const allCsvRankThroughPasses = calculateRankForMetric(filteredData, 'throughPasses');
const leagueRankThroughPasses = calculateRankForMetric(filteredData, 'throughPasses', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankThroughPassesWithMinutes = calculateRankForMetric(filteredData, 'throughPasses', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, throughPasses: Math.round(p.throughPasses * p.minutes / 90)}));
const positionRankThroughPasses = calculateRankForMetric(filteredData, 'throughPasses', p => p.position === displayLeague(selectedPlayer.position));
const positionRankThroughPassesWithMinutes = calculateRankForMetric(filteredData, 'throughPasses', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, throughPasses: Math.round(p.throughPasses * p.minutes / 90)}));
const samePositionAndLeagueThroughPasses = calculateRankForMetric(filteredData, 'throughPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueThroughPassesWithMinutes = calculateRankForMetric(filteredData, 'throughPasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, throughPasses: Math.round(p.throughPasses * p.minutes / 90)}));

// Metric: deepCompletions
const allCsvRankDeepCompletions = calculateRankForMetric(filteredData, 'deepCompletions');
const leagueRankDeepCompletions = calculateRankForMetric(filteredData, 'deepCompletions', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankDeepCompletionsWithMinutes = calculateRankForMetric(filteredData, 'deepCompletions', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, deepCompletions: Math.round(p.deepCompletions * p.minutes / 90)}));
const positionRankDeepCompletions = calculateRankForMetric(filteredData, 'deepCompletions', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDeepCompletionsWithMinutes = calculateRankForMetric(filteredData, 'deepCompletions', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, deepCompletions: Math.round(p.deepCompletions * p.minutes / 90)}));
const samePositionAndLeagueDeepCompletions = calculateRankForMetric(filteredData, 'deepCompletions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDeepCompletionsWithMinutes = calculateRankForMetric(filteredData, 'deepCompletions', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, deepCompletions: Math.round(p.deepCompletions * p.minutes / 90)}));

// Metric: progressivePasses
const allCsvRankProgressivePasses = calculateRankForMetric(filteredData, 'progressivePasses');
const leagueRankProgressivePasses = calculateRankForMetric(filteredData, 'progressivePasses', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankProgressivePassesWithMinutes = calculateRankForMetric(filteredData, 'progressivePasses', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, progressivePasses: Math.round(p.progressivePasses * p.minutes / 90)}));
const positionRankProgressivePasses = calculateRankForMetric(filteredData, 'progressivePasses', p => p.position === displayLeague(selectedPlayer.position));
const positionRankProgressivePassesWithMinutes = calculateRankForMetric(filteredData, 'progressivePasses', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, progressivePasses: Math.round(p.progressivePasses * p.minutes / 90)}));
const samePositionAndLeagueProgressivePasses = calculateRankForMetric(filteredData, 'progressivePasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueProgressivePassesWithMinutes = calculateRankForMetric(filteredData, 'progressivePasses', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, progressivePasses: Math.round(p.progressivePasses * p.minutes / 90)}));

// Metric: shotsAgainst
const allCsvRankShotsAgainst = calculateRankForMetric(filteredData, 'shotsAgainst');
const leagueRankShotsAgainst = calculateRankForMetric(filteredData, 'shotsAgainst', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankShotsAgainstWithMinutes = calculateRankForMetric(filteredData, 'shotsAgainst', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, shotsAgainst: Math.round(p.shotsAgainst * p.minutes / 90)}));
const positionRankShotsAgainst = calculateRankForMetric(filteredData, 'shotsAgainst', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShotsAgainstWithMinutes = calculateRankForMetric(filteredData, 'shotsAgainst', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, shotsAgainst: Math.round(p.shotsAgainst * p.minutes / 90)}));
const samePositionAndLeagueShotsAgainst = calculateRankForMetric(filteredData, 'shotsAgainst', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShotsAgainstWithMinutes = calculateRankForMetric(filteredData, 'shotsAgainst', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, shotsAgainst: Math.round(p.shotsAgainst * p.minutes / 90)}));

// Metric: cleanSheets
const allCsvRankCleanSheets = calculateRankForMetric(filteredData, 'cleanSheets');
const leagueRankCleanSheets = calculateRankForMetric(filteredData, 'cleanSheets', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankCleanSheetsWithMinutes = calculateRankForMetric(filteredData, 'cleanSheets');
const leagueRankCleanSheetsWithMinutes = calculateRankForMetric(filteredData, 'cleanSheets', p => p.league === displayLeague(selectedPlayer.league));
const positionRankCleanSheets = calculateRankForMetric(filteredData, 'cleanSheets', p => p.position === displayLeague(selectedPlayer.position));
const positionRankCleanSheetsWithMinutes = calculateRankForMetric(filteredData, 'cleanSheets', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueCleanSheets = calculateRankForMetric(filteredData, 'cleanSheets', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueCleanSheetsWithMinutes = calculateRankForMetric(filteredData, 'cleanSheets', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: xGAgainst
const allCsvRankXGAgainst = calculateRankForMetric(filteredData, 'xGAgainst');
const leagueRankXGAgainst = calculateRankForMetric(filteredData, 'xGAgainst', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankXGAgainstWithMinutes = calculateRankForMetric(filteredData, 'xGAgainst', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, xGAgainst: p.xGAgainst * p.minutes}));
const positionRankXGAgainst = calculateRankForMetric(filteredData, 'xGAgainst', p => p.position === displayLeague(selectedPlayer.position));
const positionRankXGAgainstWithMinutes = calculateRankForMetric(filteredData, 'xGAgainst', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, xGAgainst: p.xGAgainst * p.minutes}));
const samePositionAndLeagueXGAgainst = calculateRankForMetric(filteredData, 'xGAgainst', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueXGAgainstWithMinutes = calculateRankForMetric(filteredData, 'xGAgainst', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, xGAgainst: p.xGAgainst * p.minutes}));

// Metric: preventedGoals
const allCsvRankPreventedGoals = calculateRankForMetric(filteredData, 'preventedGoals');
const leagueRankPreventedGoals = calculateRankForMetric(filteredData, 'preventedGoals', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankPreventedGoalsWithMinutes = calculateRankForMetric(filteredData, 'preventedGoals', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, preventedGoals: p.preventedGoals * p.minutes}));
const positionRankPreventedGoals = calculateRankForMetric(filteredData, 'preventedGoals', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPreventedGoalsWithMinutes = calculateRankForMetric(filteredData, 'preventedGoals', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, preventedGoals: p.preventedGoals * p.minutes}));
const samePositionAndLeaguePreventedGoals = calculateRankForMetric(filteredData, 'preventedGoals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePreventedGoalsWithMinutes = calculateRankForMetric(filteredData, 'preventedGoals', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, preventedGoals: p.preventedGoals * p.minutes}));

// Metric: exits
const allCsvRankExits = calculateRankForMetric(filteredData, 'exits');
const leagueRankExits = calculateRankForMetric(filteredData, 'exits', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankExitsWithMinutes = calculateRankForMetric(filteredData, 'exits', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, exits: Math.round(p.exits * p.minutes / 90)}));
const positionRankExits = calculateRankForMetric(filteredData, 'exits', p => p.position === displayLeague(selectedPlayer.position));
const positionRankExitsWithMinutes = calculateRankForMetric(filteredData, 'exits', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, exits: Math.round(p.exits * p.minutes / 90)}));
const samePositionAndLeagueExits = calculateRankForMetric(filteredData, 'exits', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueExitsWithMinutes = calculateRankForMetric(filteredData, 'exits', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, exits: Math.round(p.exits * p.minutes / 90)}));
...
const leagueRankDefensiveDuelsWonPercentage = calculateRankForMetric(filteredData, 'defensiveDuelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankDefensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'defensiveDuelsWonPercentage');
const leagueRankDefensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'defensiveDuelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankDefensiveDuelsWonPercentage = calculateRankForMetric(filteredData, 'defensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDefensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'defensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueDefensiveDuelsWonPercentage = calculateRankForMetric(filteredData, 'defensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDefensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'defensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAerialDuelsWonPercentage = calculateRankForMetric(filteredData, 'aerialDuelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankAerialDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'aerialDuelsWonPercentage');
const leagueRankAerialDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'aerialDuelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAerialDuelsWonPercentage = calculateRankForMetric(filteredData, 'aerialDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAerialDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'aerialDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAerialDuelsWonPercentage = calculateRankForMetric(filteredData, 'aerialDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAerialDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'aerialDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankShotsOnTargetPercentage = calculateRankForMetric(filteredData, 'shotsOnTargetPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankShotsOnTargetPercentageWithMinutes = calculateRankForMetric(filteredData, 'shotsOnTargetPercentage');
const leagueRankShotsOnTargetPercentageWithMinutes = calculateRankForMetric(filteredData, 'shotsOnTargetPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankShotsOnTargetPercentage = calculateRankForMetric(filteredData, 'shotsOnTargetPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShotsOnTargetPercentageWithMinutes = calculateRankForMetric(filteredData, 'shotsOnTargetPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueShotsOnTargetPercentage = calculateRankForMetric(filteredData, 'shotsOnTargetPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShotsOnTargetPercentageWithMinutes = calculateRankForMetric(filteredData, 'shotsOnTargetPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankGoalConversionPercentage = calculateRankForMetric(filteredData, 'goalConversionPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankGoalConversionPercentageWithMinutes = calculateRankForMetric(filteredData, 'goalConversionPercentage');
const leagueRankGoalConversionPercentageWithMinutes = calculateRankForMetric(filteredData, 'goalConversionPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankGoalConversionPercentage = calculateRankForMetric(filteredData, 'goalConversionPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankGoalConversionPercentageWithMinutes = calculateRankForMetric(filteredData, 'goalConversionPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueGoalConversionPercentage = calculateRankForMetric(filteredData, 'goalConversionPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueGoalConversionPercentageWithMinutes = calculateRankForMetric(filteredData, 'goalConversionPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccurateCrossesPercentage = calculateRankForMetric(filteredData, 'accurateCrossesPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankAccurateCrossesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateCrossesPercentage');
const leagueRankAccurateCrossesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateCrossesPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccurateCrossesPercentage = calculateRankForMetric(filteredData, 'accurateCrossesPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccurateCrossesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateCrossesPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccurateCrossesPercentage = calculateRankForMetric(filteredData, 'accurateCrossesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccurateCrossesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateCrossesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankSuccessfulDribblesPercentage = calculateRankForMetric(filteredData, 'successfulDribblesPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankSuccessfulDribblesPercentageWithMinutes = calculateRankForMetric(filteredData, 'successfulDribblesPercentage');
const leagueRankSuccessfulDribblesPercentageWithMinutes = calculateRankForMetric(filteredData, 'successfulDribblesPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankSuccessfulDribblesPercentage = calculateRankForMetric(filteredData, 'successfulDribblesPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankSuccessfulDribblesPercentageWithMinutes = calculateRankForMetric(filteredData, 'successfulDribblesPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueSuccessfulDribblesPercentage = calculateRankForMetric(filteredData, 'successfulDribblesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueSuccessfulDribblesPercentageWithMinutes = calculateRankForMetric(filteredData, 'successfulDribblesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankOffensiveDuelsWonPercentage = calculateRankForMetric(filteredData, 'offensiveDuelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankOffensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuelsWonPercentage');
const leagueRankOffensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankOffensiveDuelsWonPercentage = calculateRankForMetric(filteredData, 'offensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankOffensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueOffensiveDuelsWonPercentage = calculateRankForMetric(filteredData, 'offensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueOffensiveDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccuratePassesPercentage = calculateRankForMetric(filteredData, 'accuratePassesPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankAccuratePassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesPercentage');
const leagueRankAccuratePassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccuratePassesPercentage = calculateRankForMetric(filteredData, 'accuratePassesPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccuratePassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccuratePassesPercentage = calculateRankForMetric(filteredData, 'accuratePassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccuratePassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccurateForwardPassesPercentage = calculateRankForMetric(filteredData, 'accurateForwardPassesPercentage', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccurateForwardPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateForwardPassesPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccurateForwardPassesPercentage = calculateRankForMetric(filteredData, 'accurateForwardPassesPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccurateForwardPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateForwardPassesPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccurateForwardPassesPercentage = calculateRankForMetric(filteredData, 'accurateForwardPassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccurateForwardPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateForwardPassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccurateShortMediumPassesPercentage = calculateRankForMetric(filteredData, 'accurateShortMediumPassesPercentage', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccurateShortMediumPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateShortMediumPassesPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccurateShortMediumPassesPercentage = calculateRankForMetric(filteredData, 'accurateShortMediumPassesPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccurateShortMediumPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateShortMediumPassesPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccurateShortMediumPassesPercentage = calculateRankForMetric(filteredData, 'accurateShortMediumPassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccurateShortMediumPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateShortMediumPassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccurateLongPassesPercentage = calculateRankForMetric(filteredData, 'accurateLongPassesPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankAccurateLongPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateLongPassesPercentage');
const leagueRankAccurateLongPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateLongPassesPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccurateLongPassesPercentage = calculateRankForMetric(filteredData, 'accurateLongPassesPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccurateLongPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateLongPassesPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccurateLongPassesPercentage = calculateRankForMetric(filteredData, 'accurateLongPassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccurateLongPassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateLongPassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccuratePassesToFinalThirdPercentage = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPercentage', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccuratePassesToFinalThirdPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccuratePassesToFinalThirdPercentage = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccuratePassesToFinalThirdPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccuratePassesToFinalThirdPercentage = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccuratePassesToFinalThirdPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccuratePassesToPenaltyAreaPercentage = calculateRankForMetric(filteredData, 'accuratePassesToPenaltyAreaPercentage', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccuratePassesToPenaltyAreaPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToPenaltyAreaPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccuratePassesToPenaltyAreaPercentage = calculateRankForMetric(filteredData, 'accuratePassesToPenaltyAreaPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccuratePassesToPenaltyAreaPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToPenaltyAreaPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccuratePassesToPenaltyAreaPercentage = calculateRankForMetric(filteredData, 'accuratePassesToPenaltyAreaPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccuratePassesToPenaltyAreaPercentageWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToPenaltyAreaPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));
...
const leagueRankAccurateProgressivePassesPercentage = calculateRankForMetric(filteredData, 'accurateProgressivePassesPercentage', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccurateProgressivePassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateProgressivePassesPercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankAccurateProgressivePassesPercentage = calculateRankForMetric(filteredData, 'accurateProgressivePassesPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccurateProgressivePassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateProgressivePassesPercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueAccurateProgressivePassesPercentage = calculateRankForMetric(filteredData, 'accurateProgressivePassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccurateProgressivePassesPercentageWithMinutes = calculateRankForMetric(filteredData, 'accurateProgressivePassesPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));

// Metric: saveRatePercentage
const allCsvRankSaveRatePercentage = calculateRankForMetric(filteredData, 'saveRatePercentage');
const leagueRankSaveRatePercentage = calculateRankForMetric(filteredData, 'saveRatePercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankSaveRatePercentageWithMinutes = calculateRankForMetric(filteredData, 'saveRatePercentage');
const leagueRankSaveRatePercentageWithMinutes = calculateRankForMetric(filteredData, 'saveRatePercentage', p => p.league === displayLeague(selectedPlayer.league), p => ({...p}));
const positionRankSaveRatePercentage = calculateRankForMetric(filteredData, 'saveRatePercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankSaveRatePercentageWithMinutes = calculateRankForMetric(filteredData, 'saveRatePercentage', p => p.position === displayLeague(selectedPlayer.position), p => ({...p}));
const samePositionAndLeagueSaveRatePercentage = calculateRankForMetric(filteredData, 'saveRatePercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueSaveRatePercentageWithMinutes = calculateRankForMetric(filteredData, 'saveRatePercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p}));

// NEW METRICS

// Metric: preAssistsPerNinety
const allCsvRankPreAssistsPerNinety = calculateRankForMetric(filteredData, 'preAssistsPerNinety');
const leagueRankPreAssistsPerNinety = calculateRankForMetric(filteredData, 'preAssistsPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankPreAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'preAssistsPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, preAssistsPerNinety: Math.round(p.preAssistsPerNinety * p.minutes / 90)}));
const positionRankPreAssistsPerNinety = calculateRankForMetric(filteredData, 'preAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPreAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'preAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, preAssistsPerNinety: Math.round(p.preAssistsPerNinety * p.minutes / 90)}));
const samePositionAndLeaguePreAssistsPerNinety = calculateRankForMetric(filteredData, 'preAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePreAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'preAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, preAssistsPerNinety: Math.round(p.preAssistsPerNinety * p.minutes / 90)}));


// Metric: duelsPerNinety
const allCsvRankDuelsPerNinety = calculateRankForMetric(filteredData, 'duelsPerNinety');
const leagueRankDuelsPerNinety = calculateRankForMetric(filteredData, 'duelsPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankDuelsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'duelsPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, duelsPerNinety: Math.round(p.duelsPerNinety * p.minutes / 90)}));
const positionRankDuelsPerNinety = calculateRankForMetric(filteredData, 'duelsPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDuelsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'duelsPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, duelsPerNinety: Math.round(p.duelsPerNinety * p.minutes / 90)}));
const samePositionAndLeagueDuelsPerNinety = calculateRankForMetric(filteredData, 'duelsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDuelsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'duelsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, duelsPerNinety: Math.round(p.duelsPerNinety * p.minutes / 90)}));
...
const leagueRankGoalsAndAssistsPerNinety = calculateRankForMetric(filteredData, 'goalsAndAssistsPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankGoalsAndAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'goalsAndAssistsPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, goalsAndAssistsPerNinety: Math.round(p.goalsAndAssistsPerNinety * p.minutes / 90)}));
const positionRankGoalsAndAssistsPerNinety = calculateRankForMetric(filteredData, 'goalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankGoalsAndAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'goalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, goalsAndAssistsPerNinety: Math.round(p.goalsAndAssistsPerNinety * p.minutes / 90)}));
const samePositionAndLeagueGoalsAndAssistsPerNinety = calculateRankForMetric(filteredData, 'goalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueGoalsAndAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'goalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, goalsAndAssistsPerNinety: Math.round(p.goalsAndAssistsPerNinety * p.minutes / 90)}));
...
const leagueRankNpGoalsAndAssistsPerNinety = calculateRankForMetric(filteredData, 'npGoalsAndAssistsPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankNpGoalsAndAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npGoalsAndAssistsPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, npGoalsAndAssistsPerNinety: Math.round(p.npGoalsAndAssistsPerNinety * p.minutes / 90)}));
const positionRankNpGoalsAndAssistsPerNinety = calculateRankForMetric(filteredData, 'npGoalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankNpGoalsAndAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npGoalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, npGoalsAndAssistsPerNinety: Math.round(p.npGoalsAndAssistsPerNinety * p.minutes / 90)}));
const samePositionAndLeagueNpGoalsAndAssistsPerNinety = calculateRankForMetric(filteredData, 'npGoalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueNpGoalsAndAssistsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npGoalsAndAssistsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, npGoalsAndAssistsPerNinety: Math.round(p.npGoalsAndAssistsPerNinety * p.minutes / 90)}));
...
const leagueRankSuccessfulDribblesPerNinety = calculateRankForMetric(filteredData, 'successfulDribblesPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankSuccessfulDribblesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'successfulDribblesPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, successfulDribblesPerNinety: Math.round(p.successfulDribblesPerNinety * p.minutes / 90)}));
const positionRankSuccessfulDribblesPerNinety = calculateRankForMetric(filteredData, 'successfulDribblesPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankSuccessfulDribblesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'successfulDribblesPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, successfulDribblesPerNinety: Math.round(p.successfulDribblesPerNinety * p.minutes / 90)}));
const samePositionAndLeagueSuccessfulDribblesPerNinety = calculateRankForMetric(filteredData, 'successfulDribblesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueSuccessfulDribblesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'successfulDribblesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, successfulDribblesPerNinety: Math.round(p.successfulDribblesPerNinety * p.minutes / 90)}));
...
const leagueRankShotsOnTargetPerNinety = calculateRankForMetric(filteredData, 'shotsOnTargetPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankShotsOnTargetPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'shotsOnTargetPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, shotsOnTargetPerNinety: Math.round(p.shotsOnTargetPerNinety * p.minutes / 90)}));
const positionRankShotsOnTargetPerNinety = calculateRankForMetric(filteredData, 'shotsOnTargetPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShotsOnTargetPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'shotsOnTargetPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, shotsOnTargetPerNinety: Math.round(p.shotsOnTargetPerNinety * p.minutes / 90)}));
const samePositionAndLeagueShotsOnTargetPerNinety = calculateRankForMetric(filteredData, 'shotsOnTargetPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShotsOnTargetPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'shotsOnTargetPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, shotsOnTargetPerNinety: Math.round(p.shotsOnTargetPerNinety * p.minutes / 90)}));
...
const leagueRankAccurateCrossesPerNinety = calculateRankForMetric(filteredData, 'accurateCrossesPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccurateCrossesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'accurateCrossesPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, accurateCrossesPerNinety: Math.round(p.accurateCrossesPerNinety * p.minutes / 90)}));
const positionRankAccurateCrossesPerNinety = calculateRankForMetric(filteredData, 'accurateCrossesPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccurateCrossesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'accurateCrossesPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, accurateCrossesPerNinety: Math.round(p.accurateCrossesPerNinety * p.minutes / 90)}));
const samePositionAndLeagueAccurateCrossesPerNinety = calculateRankForMetric(filteredData, 'accurateCrossesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccurateCrossesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'accurateCrossesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, accurateCrossesPerNinety: Math.round(p.accurateCrossesPerNinety * p.minutes / 90)}));
...
const leagueRankOffensiveDuelsWonPerNinety = calculateRankForMetric(filteredData, 'offensiveDuelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankOffensiveDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, offensiveDuelsWonPerNinety: Math.round(p.offensiveDuelsWonPerNinety * p.minutes / 90)}));
const positionRankOffensiveDuelsWonPerNinety = calculateRankForMetric(filteredData, 'offensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankOffensiveDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, offensiveDuelsWonPerNinety: Math.round(p.offensiveDuelsWonPerNinety * p.minutes / 90)}));
const samePositionAndLeagueOffensiveDuelsWonPerNinety = calculateRankForMetric(filteredData, 'offensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueOffensiveDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'offensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, offensiveDuelsWonPerNinety: Math.round(p.offensiveDuelsWonPerNinety * p.minutes / 90)}));
...
const leagueRankDefensiveDuelsWonPerNinety = calculateRankForMetric(filteredData, 'defensiveDuelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankDefensiveDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'defensiveDuelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, defensiveDuelsWonPerNinety: Math.round(p.defensiveDuelsWonPerNinety * p.minutes / 90)}));
const positionRankDefensiveDuelsWonPerNinety = calculateRankForMetric(filteredData, 'defensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDefensiveDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'defensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, defensiveDuelsWonPerNinety: Math.round(p.defensiveDuelsWonPerNinety * p.minutes / 90)}));
const samePositionAndLeagueDefensiveDuelsWonPerNinety = calculateRankForMetric(filteredData, 'defensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDefensiveDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'defensiveDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, defensiveDuelsWonPerNinety: Math.round(p.defensiveDuelsWonPerNinety * p.minutes / 90)}));
...
const leagueRankAerialDuelsWonPerNinety = calculateRankForMetric(filteredData, 'aerialDuelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAerialDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'aerialDuelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, aerialDuelsWonPerNinety: Math.round(p.aerialDuelsWonPerNinety * p.minutes / 90)}));
const positionRankAerialDuelsWonPerNinety = calculateRankForMetric(filteredData, 'aerialDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAerialDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'aerialDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, aerialDuelsWonPerNinety: Math.round(p.aerialDuelsWonPerNinety * p.minutes / 90)}));
const samePositionAndLeagueAerialDuelsWonPerNinety = calculateRankForMetric(filteredData, 'aerialDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAerialDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'aerialDuelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, aerialDuelsWonPerNinety: Math.round(p.aerialDuelsWonPerNinety * p.minutes / 90)}));


// Metric: touchesPerNinety
const allCsvRankTouchesPerNinety = calculateRankForMetric(filteredData, 'touchesPerNinety');
const leagueRankTouchesPerNinety = calculateRankForMetric(filteredData, 'touchesPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankTouchesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'touchesPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, touchesPerNinety: Math.round(p.touchesPerNinety * p.minutes / 90)}));
const positionRankTouchesPerNinety = calculateRankForMetric(filteredData, 'touchesPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankTouchesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'touchesPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, touchesPerNinety: Math.round(p.touchesPerNinety * p.minutes / 90)}));
const samePositionAndLeagueTouchesPerNinety = calculateRankForMetric(filteredData, 'touchesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueTouchesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'touchesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, touchesPerNinety: Math.round(p.touchesPerNinety * p.minutes / 90)}));
...
const leagueRankPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'passesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'passesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, passesCompletedPerNinety: Math.round(p.passesCompletedPerNinety * p.minutes / 90)}));
const positionRankPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'passesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'passesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, passesCompletedPerNinety: Math.round(p.passesCompletedPerNinety * p.minutes / 90)}));
const samePositionAndLeaguePassesCompletedPerNinety = calculateRankForMetric(filteredData, 'passesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'passesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, passesCompletedPerNinety: Math.round(p.passesCompletedPerNinety * p.minutes / 90)}));
...
const leagueRankForwardPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'forwardPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankForwardPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'forwardPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, forwardPassesCompletedPerNinety: Math.round(p.forwardPassesCompletedPerNinety * p.minutes / 90)}));
const positionRankForwardPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'forwardPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankForwardPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'forwardPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, forwardPassesCompletedPerNinety: Math.round(p.forwardPassesCompletedPerNinety * p.minutes / 90)}));
const samePositionAndLeagueForwardPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'forwardPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueForwardPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'forwardPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, forwardPassesCompletedPerNinety: Math.round(p.forwardPassesCompletedPerNinety * p.minutes / 90)}));
...
const leagueRankShortPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'shortPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankShortPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'shortPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, shortPassesCompletedPerNinety: Math.round(p.shortPassesCompletedPerNinety * p.minutes / 90)}));
const positionRankShortPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'shortPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShortPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'shortPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, shortPassesCompletedPerNinety: Math.round(p.shortPassesCompletedPerNinety * p.minutes / 90)}));
const samePositionAndLeagueShortPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'shortPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShortPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'shortPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, shortPassesCompletedPerNinety: Math.round(p.shortPassesCompletedPerNinety * p.minutes / 90)}));
...
const leagueRankLongPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'longPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankLongPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'longPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, longPassesCompletedPerNinety: Math.round(p.longPassesCompletedPerNinety * p.minutes / 90)}));
const positionRankLongPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'longPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankLongPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'longPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, longPassesCompletedPerNinety: Math.round(p.longPassesCompletedPerNinety * p.minutes / 90)}));
const samePositionAndLeagueLongPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'longPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueLongPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'longPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, longPassesCompletedPerNinety: Math.round(p.longPassesCompletedPerNinety * p.minutes / 90)}));
...
const leagueRankAccuratePassesToFinalThirdPerNinety = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankAccuratePassesToFinalThirdPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, accuratePassesToFinalThirdPerNinety: Math.round(p.accuratePassesToFinalThirdPerNinety * p.minutes / 90)}));
const positionRankAccuratePassesToFinalThirdPerNinety = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankAccuratePassesToFinalThirdPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, accuratePassesToFinalThirdPerNinety: Math.round(p.accuratePassesToFinalThirdPerNinety * p.minutes / 90)}));
const samePositionAndLeagueAccuratePassesToFinalThirdPerNinety = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueAccuratePassesToFinalThirdPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'accuratePassesToFinalThirdPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, accuratePassesToFinalThirdPerNinety: Math.round(p.accuratePassesToFinalThirdPerNinety * p.minutes / 90)}));
...
const leagueRankThroughPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'throughPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankThroughPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'throughPassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, throughPassesCompletedPerNinety: Math.round(p.throughPassesCompletedPerNinety * p.minutes / 90)}));
const positionRankThroughPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'throughPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankThroughPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'throughPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, throughPassesCompletedPerNinety: Math.round(p.throughPassesCompletedPerNinety * p.minutes / 90)}));
const samePositionAndLeagueThroughPassesCompletedPerNinety = calculateRankForMetric(filteredData, 'throughPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueThroughPassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'throughPassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, throughPassesCompletedPerNinety: Math.round(p.throughPassesCompletedPerNinety * p.minutes / 90)}));
...
const leagueRankProgressivePassesCompletedPerNinety = calculateRankForMetric(filteredData, 'progressivePassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankProgressivePassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'progressivePassesCompletedPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, progressivePassesCompletedPerNinety: Math.round(p.progressivePassesCompletedPerNinety * p.minutes / 90)}));
const positionRankProgressivePassesCompletedPerNinety = calculateRankForMetric(filteredData, 'progressivePassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankProgressivePassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'progressivePassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, progressivePassesCompletedPerNinety: Math.round(p.progressivePassesCompletedPerNinety * p.minutes / 90)}));
const samePositionAndLeagueProgressivePassesCompletedPerNinety = calculateRankForMetric(filteredData, 'progressivePassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueProgressivePassesCompletedPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'progressivePassesCompletedPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, progressivePassesCompletedPerNinety: Math.round(p.progressivePassesCompletedPerNinety * p.minutes / 90)}));
...
const leagueRankSavesPerNinety = calculateRankForMetric(filteredData, 'savesPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankSavesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'savesPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, savesPerNinety: Math.round(p.savesPerNinety * p.minutes / 90)}));
const positionRankSavesPerNinety = calculateRankForMetric(filteredData, 'savesPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankSavesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'savesPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, savesPerNinety: Math.round(p.savesPerNinety * p.minutes / 90)}));
const samePositionAndLeagueSavesPerNinety = calculateRankForMetric(filteredData, 'savesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueSavesPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'savesPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, savesPerNinety: Math.round(p.savesPerNinety * p.minutes / 90)}));
...
const leagueRankPossessionsWonMinusLostPerNinety = calculateRankForMetric(filteredData, 'possessionsWonMinusLostPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankPossessionsWonMinusLostPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'possessionsWonMinusLostPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, possessionsWonMinusLostPerNinety: Math.round(p.possessionsWonMinusLostPerNinety * p.minutes / 90)}));
const positionRankPossessionsWonMinusLostPerNinety = calculateRankForMetric(filteredData, 'possessionsWonMinusLostPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPossessionsWonMinusLostPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'possessionsWonMinusLostPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, possessionsWonMinusLostPerNinety: Math.round(p.possessionsWonMinusLostPerNinety * p.minutes / 90)}));
const samePositionAndLeaguePossessionsWonMinusLostPerNinety = calculateRankForMetric(filteredData, 'possessionsWonMinusLostPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePossessionsWonMinusLostPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'possessionsWonMinusLostPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, possessionsWonMinusLostPerNinety: Math.round(p.possessionsWonMinusLostPerNinety * p.minutes / 90)}));

// Metric: duelsWonPerNinety
const allCsvRankDuelsWonPerNinety = calculateRankForMetric(filteredData, 'duelsWonPerNinety');
const leagueRankDuelsWonPerNinety = calculateRankForMetric(filteredData, 'duelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'duelsWonPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, duelsWonPerNinety: Math.round(p.duelsWonPerNinety * p.minutes / 90)}));
const positionRankDuelsWonPerNinety = calculateRankForMetric(filteredData, 'duelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'duelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, duelsWonPerNinety: Math.round(p.duelsWonPerNinety * p.minutes / 90)}));
const samePositionAndLeagueDuelsWonPerNinety = calculateRankForMetric(filteredData, 'duelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDuelsWonPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'duelsWonPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, duelsWonPerNinety: Math.round(p.duelsWonPerNinety * p.minutes / 90)}));
...
const leagueRankProgressiveActionsPerNinety = calculateRankForMetric(filteredData, 'progressiveActionsPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankProgressiveActionsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'progressiveActionsPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, progressiveActionsPerNinety: Math.round(p.progressiveActionsPerNinety * p.minutes / 90)}));
const positionRankProgressiveActionsPerNinety = calculateRankForMetric(filteredData, 'progressiveActionsPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankProgressiveActionsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'progressiveActionsPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, progressiveActionsPerNinety: Math.round(p.progressiveActionsPerNinety * p.minutes / 90)}));
const samePositionAndLeagueProgressiveActionsPerNinety = calculateRankForMetric(filteredData, 'progressiveActionsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueProgressiveActionsPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'progressiveActionsPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, progressiveActionsPerNinety: Math.round(p.progressiveActionsPerNinety * p.minutes / 90)}));


// Metric: duelsWonPercentage
const allCsvRankDuelsWonPercentage = calculateRankForMetric(filteredData, 'duelsWonPercentage');
const leagueRankDuelsWonPercentage = calculateRankForMetric(filteredData, 'duelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'duelsWonPercentage');
const leagueRankDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'duelsWonPercentage', p => p.league === displayLeague(selectedPlayer.league));
const positionRankDuelsWonPercentage = calculateRankForMetric(filteredData, 'duelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'duelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueDuelsWonPercentage = calculateRankForMetric(filteredData, 'duelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDuelsWonPercentageWithMinutes = calculateRankForMetric(filteredData, 'duelsWonPercentage', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: possessionPlusMinus
const allCsvRankPossessionPlusMinus = calculateRankForMetric(filteredData, 'possessionPlusMinus');
const leagueRankPossessionPlusMinus = calculateRankForMetric(filteredData, 'possessionPlusMinus', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankPossessionPlusMinusWithMinutes = calculateRankForMetric(filteredData, 'possessionPlusMinus');
const leagueRankPossessionPlusMinusWithMinutes = calculateRankForMetric(filteredData, 'possessionPlusMinus', p => p.league === displayLeague(selectedPlayer.league));
const positionRankPossessionPlusMinus = calculateRankForMetric(filteredData, 'possessionPlusMinus', p => p.position === displayLeague(selectedPlayer.position));
const positionRankPossessionPlusMinusWithMinutes = calculateRankForMetric(filteredData, 'possessionPlusMinus', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeaguePossessionPlusMinus = calculateRankForMetric(filteredData, 'possessionPlusMinus', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeaguePossessionPlusMinusWithMinutes = calculateRankForMetric(filteredData, 'possessionPlusMinus', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: forwardPassRatio
const allCsvRankForwardPassRatio = calculateRankForMetric(filteredData, 'forwardPassRatio');
const leagueRankForwardPassRatio = calculateRankForMetric(filteredData, 'forwardPassRatio', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankForwardPassRatioWithMinutes = calculateRankForMetric(filteredData, 'forwardPassRatio');
const leagueRankForwardPassRatioWithMinutes = calculateRankForMetric(filteredData, 'forwardPassRatio', p => p.league === displayLeague(selectedPlayer.league));
const positionRankForwardPassRatio = calculateRankForMetric(filteredData, 'forwardPassRatio', p => p.position === displayLeague(selectedPlayer.position));
const positionRankForwardPassRatioWithMinutes = calculateRankForMetric(filteredData, 'forwardPassRatio', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueForwardPassRatio = calculateRankForMetric(filteredData, 'forwardPassRatio', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueForwardPassRatioWithMinutes = calculateRankForMetric(filteredData, 'forwardPassRatio', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: xAPer100Passes
const allCsvRankXAPer100Passes = calculateRankForMetric(filteredData, 'xAPer100Passes');
const leagueRankXAPer100Passes = calculateRankForMetric(filteredData, 'xAPer100Passes', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankXAPer100PassesWithMinutes = calculateRankForMetric(filteredData, 'xAPer100Passes');
const leagueRankXAPer100PassesWithMinutes = calculateRankForMetric(filteredData, 'xAPer100Passes', p => p.league === displayLeague(selectedPlayer.league));
const positionRankXAPer100Passes = calculateRankForMetric(filteredData, 'xAPer100Passes', p => p.position === displayLeague(selectedPlayer.position));
const positionRankXAPer100PassesWithMinutes = calculateRankForMetric(filteredData, 'xAPer100Passes', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueXAPer100Passes = calculateRankForMetric(filteredData, 'xAPer100Passes', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueXAPer100PassesWithMinutes = calculateRankForMetric(filteredData, 'xAPer100Passes', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));


// Metric: chanceCreationRatio
const allCsvRankChanceCreationRatio = calculateRankForMetric(filteredData, 'chanceCreationRatio');
const leagueRankChanceCreationRatio = calculateRankForMetric(filteredData, 'chanceCreationRatio', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankChanceCreationRatioWithMinutes = calculateRankForMetric(filteredData, 'chanceCreationRatio');
const leagueRankChanceCreationRatioWithMinutes = calculateRankForMetric(filteredData, 'chanceCreationRatio', p => p.league === displayLeague(selectedPlayer.league));
const positionRankChanceCreationRatio = calculateRankForMetric(filteredData, 'chanceCreationRatio', p => p.position === displayLeague(selectedPlayer.position));
const positionRankChanceCreationRatioWithMinutes = calculateRankForMetric(filteredData, 'chanceCreationRatio', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueChanceCreationRatio = calculateRankForMetric(filteredData, 'chanceCreationRatio', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueChanceCreationRatioWithMinutes = calculateRankForMetric(filteredData, 'chanceCreationRatio', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));


// Metric: npxGPerShot
const allCsvRankNpxGPerShot = calculateRankForMetric(filteredData, 'npxGPerShot');
const leagueRankNpxGPerShot = calculateRankForMetric(filteredData, 'npxGPerShot', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankNpxGPerShotWithMinutes = calculateRankForMetric(filteredData, 'npxGPerShot');
const leagueRankNpxGPerShotWithMinutes = calculateRankForMetric(filteredData, 'npxGPerShot', p => p.league === displayLeague(selectedPlayer.league));
const positionRankNpxGPerShot = calculateRankForMetric(filteredData, 'npxGPerShot', p => p.position === displayLeague(selectedPlayer.position));
const positionRankNpxGPerShotWithMinutes = calculateRankForMetric(filteredData, 'npxGPerShot', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueNpxGPerShot = calculateRankForMetric(filteredData, 'npxGPerShot', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueNpxGPerShotWithMinutes = calculateRankForMetric(filteredData, 'npxGPerShot', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
...
const leagueRankProgressiveActionRate = calculateRankForMetric(filteredData, 'progressiveActionRate', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankProgressiveActionRateWithMinutes = calculateRankForMetric(filteredData, 'progressiveActionRate');
const leagueRankProgressiveActionRateWithMinutes = calculateRankForMetric(filteredData, 'progressiveActionRate', p => p.league === displayLeague(selectedPlayer.league));
const positionRankProgressiveActionRate = calculateRankForMetric(filteredData, 'progressiveActionRate', p => p.position === displayLeague(selectedPlayer.position));
const positionRankProgressiveActionRateWithMinutes = calculateRankForMetric(filteredData, 'progressiveActionRate', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueProgressiveActionRate = calculateRankForMetric(filteredData, 'progressiveActionRate', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueProgressiveActionRateWithMinutes = calculateRankForMetric(filteredData, 'progressiveActionRate', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
...
const leagueRankProgressivePassesPAdj = calculateRankForMetric(filteredData, 'progressivePassesPAdj', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankProgressivePassesPAdjWithMinutes = calculateRankForMetric(filteredData, 'progressivePassesPAdj');
const leagueRankProgressivePassesPAdjWithMinutes = calculateRankForMetric(filteredData, 'progressivePassesPAdj', p => p.league === displayLeague(selectedPlayer.league));
const positionRankProgressivePassesPAdj = calculateRankForMetric(filteredData, 'progressivePassesPAdj', p => p.position === displayLeague(selectedPlayer.position));
const positionRankProgressivePassesPAdjWithMinutes = calculateRankForMetric(filteredData, 'progressivePassesPAdj', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueProgressivePassesPAdj = calculateRankForMetric(filteredData, 'progressivePassesPAdj', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueProgressivePassesPAdjWithMinutes = calculateRankForMetric(filteredData, 'progressivePassesPAdj', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
...
const leagueRankBallCarryingFrequency = calculateRankForMetric(filteredData, 'ballCarryingFrequency', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankBallCarryingFrequencyWithMinutes = calculateRankForMetric(filteredData, 'ballCarryingFrequency');
const leagueRankBallCarryingFrequencyWithMinutes = calculateRankForMetric(filteredData, 'ballCarryingFrequency', p => p.league === displayLeague(selectedPlayer.league));
const positionRankBallCarryingFrequency = calculateRankForMetric(filteredData, 'ballCarryingFrequency', p => p.position === displayLeague(selectedPlayer.position));
const positionRankBallCarryingFrequencyWithMinutes = calculateRankForMetric(filteredData, 'ballCarryingFrequency', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueBallCarryingFrequency = calculateRankForMetric(filteredData, 'ballCarryingFrequency', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueBallCarryingFrequencyWithMinutes = calculateRankForMetric(filteredData, 'ballCarryingFrequency', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: xGPer100Touches
const allCsvRankXGPer100Touches = calculateRankForMetric(filteredData, 'xGPer100Touches');
const leagueRankXGPer100Touches = calculateRankForMetric(filteredData, 'xGPer100Touches', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankXGPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'xGPer100Touches');
const leagueRankXGPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'xGPer100Touches', p => p.league === displayLeague(selectedPlayer.league));
const positionRankXGPer100Touches = calculateRankForMetric(filteredData, 'xGPer100Touches', p => p.position === displayLeague(selectedPlayer.position));
const positionRankXGPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'xGPer100Touches', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueXGPer100Touches = calculateRankForMetric(filteredData, 'xGPer100Touches', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueXGPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'xGPer100Touches', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: shotFrequency
const allCsvRankShotFrequency = calculateRankForMetric(filteredData, 'shotFrequency');
const leagueRankShotFrequency = calculateRankForMetric(filteredData, 'shotFrequency', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankShotFrequencyWithMinutes = calculateRankForMetric(filteredData, 'shotFrequency');
const leagueRankShotFrequencyWithMinutes = calculateRankForMetric(filteredData, 'shotFrequency', p => p.league === displayLeague(selectedPlayer.league));
const positionRankShotFrequency = calculateRankForMetric(filteredData, 'shotFrequency', p => p.position === displayLeague(selectedPlayer.position));
const positionRankShotFrequencyWithMinutes = calculateRankForMetric(filteredData, 'shotFrequency', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueShotFrequency = calculateRankForMetric(filteredData, 'shotFrequency', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueShotFrequencyWithMinutes = calculateRankForMetric(filteredData, 'shotFrequency', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
...
const leagueRankDribblesPerHundredTouches = calculateRankForMetric(filteredData, 'dribblesPerHundredTouches', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankDribblesPerHundredTouchesWithMinutes = calculateRankForMetric(filteredData, 'dribblesPerHundredTouches');
const leagueRankDribblesPerHundredTouchesWithMinutes = calculateRankForMetric(filteredData, 'dribblesPerHundredTouches', p => p.league === displayLeague(selectedPlayer.league));
const positionRankDribblesPerHundredTouches = calculateRankForMetric(filteredData, 'dribblesPerHundredTouches', p => p.position === displayLeague(selectedPlayer.position));
const positionRankDribblesPerHundredTouchesWithMinutes = calculateRankForMetric(filteredData, 'dribblesPerHundredTouches', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueDribblesPerHundredTouches = calculateRankForMetric(filteredData, 'dribblesPerHundredTouches', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueDribblesPerHundredTouchesWithMinutes = calculateRankForMetric(filteredData, 'dribblesPerHundredTouches', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));

// Metric: goalsPer100Touches
const allCsvRankGoalsPer100Touches = calculateRankForMetric(filteredData, 'goalsPer100Touches');
const leagueRankGoalsPer100Touches = calculateRankForMetric(filteredData, 'goalsPer100Touches', p => p.league === displayLeague(selectedPlayer.league));
const allCsvRankGoalsPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'goalsPer100Touches');
const leagueRankGoalsPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'goalsPer100Touches', p => p.league === displayLeague(selectedPlayer.league));
const positionRankGoalsPer100Touches = calculateRankForMetric(filteredData, 'goalsPer100Touches', p => p.position === displayLeague(selectedPlayer.position));
const positionRankGoalsPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'goalsPer100Touches', p => p.position === displayLeague(selectedPlayer.position));
const samePositionAndLeagueGoalsPer100Touches = calculateRankForMetric(filteredData, 'goalsPer100Touches', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueGoalsPer100TouchesWithMinutes = calculateRankForMetric(filteredData, 'goalsPer100Touches', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
...
const leagueRankGoalsMinusxGPerNinety = calculateRankForMetric(filteredData, 'goalsMinusxGPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankGoalsMinusxGPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'goalsMinusxGPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, goalsMinusxGPerNinety: p.goalsMinusxGPerNinety * p.minutes / 90}));
const positionRankGoalsMinusxGPerNinety = calculateRankForMetric(filteredData, 'goalsMinusxGPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankGoalsMinusxGPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'goalsMinusxGPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, goalsMinusxGPerNinety: p.goalsMinusxGPerNinety * p.minutes / 90}));
const samePositionAndLeagueGoalsMinusxGPerNinety = calculateRankForMetric(filteredData, 'goalsMinusxGPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueGoalsMinusxGPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'goalsMinusxGPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, goalsMinusxGPerNinety: p.goalsMinusxGPerNinety * p.minutes / 90}));


// Metric: npxGAndxAPerNinety
const allCsvRankNpxGAndxAPerNinety = calculateRankForMetric(filteredData, 'npxGAndxAPerNinety');
const leagueRankNpxGAndxAPerNinety = calculateRankForMetric(filteredData, 'npxGAndxAPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankNpxGAndxAPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npxGAndxAPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, npxGAndxAPerNinety: p.npxGAndxAPerNinety * p.minutes / 90}));
const positionRankNpxGAndxAPerNinety = calculateRankForMetric(filteredData, 'npxGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankNpxGAndxAPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npxGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, npxGAndxAPerNinety: p.npxGAndxAPerNinety * p.minutes / 90}));
const samePositionAndLeagueNpxGAndxAPerNinety = calculateRankForMetric(filteredData, 'npxGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueNpxGAndxAPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npxGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, npxGAndxAPerNinety: p.npxGAndxAPerNinety * p.minutes / 90}));


// Metric: npxGPerNinety
const allCsvRankNpxGPerNinety = calculateRankForMetric(filteredData, 'npxGPerNinety');
const leagueRankNpxGPerNinety = calculateRankForMetric(filteredData, 'npxGPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankNpxGPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npxGPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, npxGPerNinety: p.npxGPerNinety * p.minutes / 90}));
const positionRankNpxGPerNinety = calculateRankForMetric(filteredData, 'npxGPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankNpxGPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npxGPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, npxGPerNinety: p.npxGPerNinety * p.minutes / 90}));
const samePositionAndLeagueNpxGPerNinety = calculateRankForMetric(filteredData, 'npxGPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueNpxGPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'npxGPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, npxGPerNinety: p.npxGPerNinety * p.minutes / 90}));


// Metric: xGAndxAPerNinety
const allCsvRankXGAndxAPerNinety = calculateRankForMetric(filteredData, 'xGAndxAPerNinety');
const leagueRankXGAndxAPerNinety = calculateRankForMetric(filteredData, 'xGAndxAPerNinety', p => p.league === displayLeague(selectedPlayer.league));
...
const leagueRankXGAndxAPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'xGAndxAPerNinety', p => p.league === displayLeague(selectedPlayer.league), p => ({...p, xGAndxAPerNinety: p.xGAndxAPerNinety * p.minutes / 90}));
const positionRankXGAndxAPerNinety = calculateRankForMetric(filteredData, 'xGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position));
const positionRankXGAndxAPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'xGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position), p => ({...p, xGAndxAPerNinety: p.xGAndxAPerNinety * p.minutes / 90}));
const samePositionAndLeagueXGAndxAPerNinety = calculateRankForMetric(filteredData, 'xGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league));
const samePositionAndLeagueXGAndxAPerNinetyWithMinutes = calculateRankForMetric(filteredData, 'xGAndxAPerNinety', p => p.position === displayLeague(selectedPlayer.position) && p.league === displayLeague(selectedPlayer.league), p => ({...p, xGAndxAPerNinety: p.xGAndxAPerNinety * p.minutes / 90}));
...
if (selectedAge && selectedAge !== '') {
    ...
    const metricsData = [
        ...
        if (displayLeague(selectedPlayer.position) in positionOrder) {
            const orderedMetricsData = positionOrder[displayLeague(selectedPlayer.position)].map(metricName => metricsData.find(metric => metric.name === metricName));
...
const metricsHTML = metricsData.map(metric => {
    const selectedAge = parseInt(ageSelect.value);
    const filteredData1 = parseCSV(csvData).filter(player => player.position === displayLeague(selectedPlayer.position) && player.league === displayLeague(selectedPlayer.league) &&
...
const filteredData1 = parseCSV(csvData).filter(player => 
    player.position === displayLeague(selectedPlayer.position) && 
    player.league === displayLeague(selectedPlayer.league) &&
    (!selectedAge || player.age <= selectedAge)
);
...
const playerPosition = displayLeague(selectedPlayer.position);
...
<h3><b>${selectedPlayer.player} (${selectedPlayer.team}, ${selectedPlayer.age})</b></h3>
          <h1><i>vs ${displayLeague(selectedPlayer.league)} ${titleSuffix} ${displayLeague(selectedPlayer.position)}s, per 90</i></h1>
...
const metricsHTML = metricsData.map(metric => {
    const selectedAge = parseInt(ageSelect.value);
        const filteredData2 = parseCSV(csvData).filter(player => player.position === displayLeague(selectedPlayer.position) && player.league === displayLeague(selectedPlayer.league) &&
    (!selectedAge || player.age <= selectedAge) );
    const playerRank = metric.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
...
<img class="logo-image2" src="https://datamb.football/logopro.png" alt="">
<h3><b>${selectedPlayer.player} (${selectedPlayer.team}, ${selectedPlayer.age})</b></h3>
          <h1><i>vs ${displayLeague(selectedPlayer.league)} ${titleSuffix} ${displayLeague(selectedPlayer.position)}s</i></h1>
...
const rankB = b.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
    ...
    const metricsHTML = metricsData.map(metric => {
    const selectedAge = parseInt(ageSelect.value);
        const filteredData3 = parseCSV(csvData).filter(player => player.position === displayLeague(selectedPlayer.position) &&
    (!selectedAge || player.age <= selectedAge) );
    const playerRank = metric.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
...
<img class="logo-image2" src="https://datamb.football/logopro.png" alt="">
<h3><b>${selectedPlayer.player} (${selectedPlayer.team}, ${selectedPlayer.age})</b></h3>
          <h1><i>vs All Leagues ${titleSuffix} ${displayLeague(selectedPlayer.position)}s, per 90</i></h1>
...
const rankB = b.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
    ...
    const metricsHTML = metricsData.map(metric => {
    const selectedAge = parseInt(ageSelect.value);
        const filteredData4 = parseCSV(csvData).filter(player => player.position === displayLeague(selectedPlayer.position) &&
    (!selectedAge || player.age <= selectedAge) );
    const playerRank = metric.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
...
<img class="logo-image2" src="https://datamb.football/logopro.png" alt="">
<h3><b>${selectedPlayer.player} (${selectedPlayer.team}, ${selectedPlayer.age})</b></h3>
          <h1><i>vs All Leagues ${titleSuffix} ${displayLeague(selectedPlayer.position)}s</i></h1>
...
const rankB = b.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
 ...
 const metricsHTML = metricsData.map(metric => {
    const selectedAge = parseInt(ageSelect.value);
        const filteredData5 = parseCSV(csvData).filter(player => player.league === displayLeague(selectedPlayer.league) &&
    (!selectedAge || player.age <= selectedAge) );
    const playerRank = metric.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
...
<img class="logo-image2" src="https://datamb.football/logopro.png" alt="">
<h3><b>${selectedPlayer.player} (${selectedPlayer.team}, ${selectedPlayer.age})</b></h3>
          <h1><i>vs ${displayLeague(selectedPlayer.league)} ${titleSuffix} players, per 90</i></h1>
...
const rankB = b.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
    ...
    const metricsHTML = metricsData.map(metric => {
    const selectedAge = parseInt(ageSelect.value);
        const filteredData6 = parseCSV(csvData).filter(player => player.league === displayLeague(selectedPlayer.league) &&
    (!selectedAge || player.age <= selectedAge) );
    const playerRank = metric.data.find(rank => rank.player === selectedPlayer.player && rank.team === selectedPlayer.team).rank;
...
<img class="logo-image2" src="https://datamb.football/logopro.png" alt="">
<h3><b>${selectedPlayer.player} (${selectedPlayer.team}, ${selectedPlayer.age})</b></h3>
          <h1><i>vs ${displayLeague(selectedPlayer.league)} ${titleSuffix} players</i></h1>
